...
function ServicesHero({ setPage, mobile }) {
  ...
  const pad = mobile ? '120px 20px 64px' : '180px 56px 100px';

  return (
    <section data-screen-label="Services Hero" style={{ background: 'var(--dark)', padding: pad, position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ maxWidth: 1160, margin: '0 auto', position: 'relative' }}>
        ...
        <h1 style={{ ...tr(0.38), fontFamily: "'Cormorant Garamond',serif", fontSize: 'clamp(36px,6vw,76px)', fontWeight: 300, color: '#FFF', lineHeight: 1.06, marginBottom: 24, maxWidth: 700 }}>
          Expertise digitale complète pour les professions du droit
        </h1>
        <p style={{ ...tr(0.55), fontSize: mobile ? 14 : 16, lineHeight: 1.85, color: 'rgba(255,255,255,0.42)', maxWidth: 520, marginBottom: 40 }}>
        ...
        <div style={{ ...tr(0.7), display: 'flex', gap: 12, flexWrap: 'wrap', flexDirection: mobile ? 'column' : 'row' }}>
          ...
          <button className="btn btn-outline-w" style={mobile ? { width: '100%', maxWidth: 320, justifyContent: 'center' } : {}}>Télécharger notre catalogue</button>
        </div>
      </div>
    </section>
  );
...
function ServiceBlock({ svc, idx, setPage, mobile }) {
  const isEven = idx % 2 === 0;
  const pad = mobile ? '64px 20px' : '96px 56px';
  // On mobile always LTR stacked, on desktop alternate
  const gridCols = mobile ? '1fr' : '1fr 1fr';
  const dir = (!mobile && !isEven) ? 'rtl' : 'ltr';

  return (
    <section data-screen-label={`Service ${svc.num}`} style={{ padding: pad, background: isEven ? 'var(--bg)' : 'var(--bg-alt)' }}>
      <div style={{ maxWidth: 1160, margin: '0 auto', display: 'grid', gridTemplateColumns: gridCols, gap: mobile ? 36 : 80, alignItems: 'center', direction: dir }}>
        {/* Text */}
        <div style={{ direction: 'ltr' }}>
          ...
          <Reveal d={1}>
            <h2 style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 'clamp(26px,3vw,42px)', fontWeight: 400, lineHeight: 1.12, color: 'var(--dark)', marginBottom: 12 }}>{svc.title}</h2>
          </Reveal>
          <Reveal d={2}>
          ...
          <Reveal d={3}>
            ...
            <button className="btn btn-gold" style={mobile ? { width: '100%', maxWidth: 320, justifyContent: 'center' } : {}} onClick={() => { setPage('contact'); window.scrollTo({ top: 0 }); }}>Demander un devis</button>
          </Reveal>
        </div>
        {/* Visual */}
        <div style={{ direction: 'ltr' }}>
...
function ServicesPage({ setPage }) {
  ...
  const ctaPad = mobile ? '72px 20px' : '100px 56px';
  return (
    <div>
      <ServicesHero setPage={setPage} mobile={mobile} />
      {SERVICE_DATA.map((svc, i) => <ServiceBlock key={svc.type} svc={svc} idx={i} setPage={setPage} mobile={mobile} />)}
      {/* Services CTA */}
      <section style={{ background: 'var(--dark)', padding: ctaPad, textAlign: 'center' }}>
        <div style={{ maxWidth: 600, margin: '0 auto' }}>
          <Reveal><STag style={{ justifyContent: 'center' }}>Étape suivante</STag></Reveal>
          <Reveal d={1}><h2 style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 'clamp(28px,4vw,52px)', fontWeight: 400, lineHeight: 1.1, color: '#FFF', marginBottom: 18 }}>Quel service correspond à votre besoin ?</h2></Reveal>
          <Reveal d={2}><p style={{ fontSize: mobile ? 14 : 15, lineHeight: 1.85, color: 'rgba(255,255,255,0.38)', marginBottom: 40 }}>Lors de votre consultation gratuite, nous identifions ensemble les leviers prioritaires pour votre structure.</p></Reveal>
          <Reveal d={3} style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap', flexDirection: mobile ? 'column' : 'row', alignItems: 'center' }}>
            <button className="btn btn-gold" style={mobile ? { width: '100%', maxWidth: 320, justifyContent: 'center' } : {}} onClick={() => { setPage('contact'); window.scrollTo({ top: 0 }); }}>Consultation gratuite</button>
            <button className="btn btn-outline-w" style={mobile ? { width: '100%', maxWidth: 320, justifyContent: 'center' } : {}}>Télécharger notre brochure</button>
          </Reveal>
        </div>
      </section>
    </div>
...
function ContactPage() {
      ...
      {options.map(opt => {
        ...
        return (
          <button key={opt} onClick={() => multi ? toggleService(opt) : set(field, opt)} style={{
            padding: '9px 14px', fontSize: 12.5, fontWeight: active ? 600 : 400,
  ...
  const heroPad = mobile ? '110px 20px 56px' : '180px 56px 80px';
  const formPad = mobile ? '48px 20px 80px' : '80px 56px 120px';

  return (
    <div style={{ minHeight: '100vh' }}>
      {/* Hero */}
      <section data-screen-label="Contact Hero" style={{ background: 'var(--dark)', padding: heroPad, position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', inset: 0, backgroundImage: 'linear-gradient(rgba(196,160,90,0.025) 1px,transparent 1px),linear-gradient(90deg,rgba(196,160,90,0.025) 1px,transparent 1px)', backgroundSize: '80px 80px', pointerEvents: 'none' }} />
        <div style={{ maxWidth: 1160, margin: '0 auto', position: 'relative', display: 'grid', gridTemplateColumns: mobile ? '1fr' : '1fr 1fr', gap: mobile ? 40 : 80, alignItems: 'center' }}>
          <div>
            <div style={{ ...tr(0.2) }}><div className="stag" style={{ color: 'var(--gold)' }}>Contact</div></div>
            <h1 style={{ ...tr(0.38), fontFamily: "'Cormorant Garamond',serif", fontSize: 'clamp(34px,5vw,64px)', fontWeight: 300, color: '#FFF', lineHeight: 1.08, marginBottom: 20 }}>
              Parlons de votre projet
            </h1>
            <p style={{ ...tr(0.55), fontSize: mobile ? 14 : 15.5, lineHeight: 1.85, color: 'rgba(255,255,255,0.42)', marginBottom: 36 }}>
          ...
          {/* Contact info */}
          <div style={{ ...tr(0.5) }}>
            <div style={{ border: '1px solid rgba(196,160,90,0.12)', padding: mobile ? '28px 20px' : '36px 32px' }}>
      ...
      {/* Multi-step form */}
      <section data-screen-label="Contact Form" style={{ padding: formPad, background: 'var(--bg)' }}>
        <div style={{ maxWidth: 720, margin: '0 auto' }}>
          {!done ? (
            <>
              {/* Progress stepper */}
              <div style={{ display: 'flex', alignItems: 'center', marginBottom: mobile ? 36 : 56, gap: 0 }}>
                  ...
                  <div key={i} style={{ display: 'flex', alignItems: 'center', flex: i < STEP_LABELS.length - 1 ? 1 : 'none' }}>
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
                      <div style={{
                        ...
                        border: `1px solid ${i <= step ? 'var(--gold)' : 'rgba(0,0,0,0.1)'}`,
                      ...
                      {!mobile && <div style={{ fontSize: 11, fontWeight: 500, letterSpacing: '0.06em', color: i === step ? 'var(--text)' : 'var(--text-light)', whiteSpace: 'nowrap' }}>{label}</div>}
                    </div>
                    {i < STEP_LABELS.length - 1 && <div style={{ flex: 1, height: 1, background: i < step ? 'var(--gold)' : 'rgba(0,0,0,0.08)', margin: `0 ${mobile ? 8 : 12}px`, marginBottom: mobile ? 0 : 24, transition: 'background 0.3s' }} />}
                  </div>
                ))}
              </div>

              {/* Step content */}
              <div style={{ background: 'var(--white)', padding: mobile ? '32px 20px' : '48px 44px', border: '1px solid rgba(0,0,0,0.06)' }}>
                {step === 0 && (
                  <div>
                    <h3 style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: mobile ? 24 : 28, fontWeight: 400, marginBottom: 8, color: 'var(--dark)' }}>Votre profil professionnel</h3>
                    <p style={{ fontSize: 14, color: 'var(--text-mid)', marginBottom: 32, lineHeight: 1.7 }}>Afin de vous proposer les meilleures solutions, aidez-nous à comprendre votre structure.</p>
                ...
                {step === 1 && (
                  <div>
                    <h3 style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: mobile ? 24 : 28, fontWeight: 400, marginBottom: 8, color: 'var(--dark)' }}>Votre projet digital</h3>
                    <p style={{ fontSize: 14, color: 'var(--text-mid)', marginBottom: 32, lineHeight: 1.7 }}>Quels services vous intéressent ? Plusieurs choix possibles.</p>
                    <div style={{ marginBottom: 28 }}>
                ...
                {step === 2 && (
                  <div>
                    <h3 style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: mobile ? 24 : 28, fontWeight: 400, marginBottom: 8, color: 'var(--dark)' }}>Vos coordonnées</h3>
                    <p style={{ fontSize: 14, color: 'var(--text-mid)', marginBottom: 32, lineHeight: 1.7 }}>Dernière étape — nous vous contacterons sous 24h pour planifier votre consultation.</p>
                    <div className="form-grid-2" style={{ display: 'grid', gridTemplateColumns: mobile ? '1fr' : '1fr 1fr', gap: 20, marginBottom: 20 }}>
                      <div>
                ...
                {/* Navigation */}
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 36, paddingTop: 28, borderTop: '1px solid rgba(0,0,0,0.06)', flexWrap: 'wrap', gap: 12 }}>
                  {step > 0 ? (
                  ...
                  <button
                    ...
                    disabled={!canNext()}
                    style={{ opacity: canNext() ? 1 : 0.4, cursor: canNext() ? 'pointer' : 'not-allowed', marginLeft: 'auto' }}
            ...
            /* Success */
            <div style={{ background: 'var(--white)', padding: mobile ? '48px 24px' : '72px 44px', border: '1px solid rgba(0,0,0,0.06)', textAlign: 'center' }}>
              ...
              <h3 style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: mobile ? 28 : 36, fontWeight: 400, marginBottom: 14, color: 'var(--dark)' }}>Demande envoyée avec succès</h3>
              ...
              <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
                <div style={{ padding: '12px 16px', background: 'var(--gold-bg)', border: '1px solid var(--gold-light)', fontSize: 13, color: 'var(--gold-dark)' }}>📋 Récapitulatif envoyé par email</div>
                <div style={{ padding: '12px 16px', background: 'var(--gold-bg)', border: '1px solid var(--gold-light)', fontSize: 13, color: 'var(--gold-dark)' }}>📞 Appel sous 24h ouvrées</div>
              </div>
