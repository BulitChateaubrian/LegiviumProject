...
function HeroSection({ setPage }) {
  ...
  return (
    <section data-screen-label="Hero" style={{ height: '100vh', minHeight: 680, background: 'radial-gradient(ellipse at 22% 65%, #1c1428 0%, #0C0C0F 62%)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '0 40px', position: 'relative', overflow: 'hidden' }}>

      {/* Subtle grid */}
      <div style={{ position: 'absolute', inset: 0, backgroundImage: 'linear-gradient(rgba(196,160,90,0.025) 1px, transparent 1px), linear-gradient(90deg, rgba(196,160,90,0.025) 1px, transparent 1px)', backgroundSize: '90px 90px', pointerEvents: 'none' }} />

      {/* Decorative rings */}
      <div style={{ position: 'absolute', top: '15%', right: '8%', opacity: 0.06, pointerEvents: 'none' }}>
      ...
      <div style={{ textAlign: 'center', maxWidth: 860, position: 'relative' }}>
        ...
        <h1 style={{ fontFamily: "'Cormorant Garamond',serif", lineHeight: 1.06, marginBottom: 0 }}>
          ...
          <div style={{ ...tr(0.55), fontSize: 'clamp(48px,7.5vw,90px)', fontWeight: 400, fontStyle: 'italic', color: 'rgba(255,255,255,0.75)', letterSpacing: '-0.01em' }}>
        ...
        {/* Subtitle */}
        <p style={{ ...tr(1.0), fontSize: 16, lineHeight: 1.85, color: 'rgba(255,255,255,0.42)', maxWidth: 520, margin: '0 auto 44px', fontWeight: 300, letterSpacing: '0.01em' }}>
...
function StatsSection() {
  ...
  const stats = [
    { num: '120+', label: 'Projets livrés', sub: 'pour des professionnels du droit' },
    { num: '×3',   label: 'Leads générés', sub: 'en moyenne après 6 mois' },
    { num: '98%',  label: 'Satisfaction client', sub: 'sur l\'ensemble de nos projets' },
  ];
  return (
    <section ref={ref} style={{ background: 'var(--bg-alt)', padding: '100px 56px' }}>
      <div style={{ maxWidth: 1160, margin: '0 auto' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 2 }}>
          {stats.map((s, i) => (
            <div key={i} className={`reveal ${vis ? 'vis' : ''} d${i + 1}`} style={{ textAlign: 'center', padding: '56px 40px', background: 'var(--white)' }}>
  ...
  return (
    <section style={{ padding: '120px 56px', background: 'var(--bg-alt)' }}>
      <div style={{ maxWidth: 1160, margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 2 }}>
          {services.map((s, i) => (
            <Reveal key={s.type} d={Math.min(i + 1, 6)}>
              <div className="scard" onClick={() => { setPage('services'); window.scrollTo({ top: 0 }); }}>
                ...
                <p style={{ fontSize: 13.5, lineHeight: 1.75, color: 'var(--text-mid)' }}>{s.desc}</p>
    ...
    <section style={{ padding: '120px 56px', background: 'var(--bg)' }}>
      <div style={{ maxWidth: 1160, margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 2 }}>
          {steps.map((s, i) => (
            <Reveal key={s.n} d={i + 1} style={{ background: i % 2 === 0 ? 'var(--white)' : 'var(--bg-alt)', padding: '44px 32px' }}>
              <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 48, fontWeight: 300, color: 'rgba(196,160,90,0.25)', lineHeight: 1, marginBottom: 20 }}>{s.n}</div>
              <div style={{ fontSize: 15, fontWeight: 600, color: 'var(--dark)', marginBottom: 12 }}>{s.title}</div>
              <div style={{ fontSize: 13.5, lineHeight: 1.75, color: 'var(--text-mid)' }}>{s.desc}</div>
            </Reveal>
          ))}
        </div>
      </div>
